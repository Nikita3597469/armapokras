...
export default function PortfolioPage() {
  const [activeFilter, setActiveFilter] =
    useState<(typeof FILTERS)[number]>("Все");
      ...
      <div className="container-custom">
        ...
        <nav aria-label="Breadcrumb" className="mb-8">
          <ol className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <li className="font-medium text-primary">Портфолио</li>
          </ol>
        </nav>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((item) => (
            <div
              key={item.id}
              className="card group animate-[fadeIn_0.3s_ease-out]"
            >
              ...
              <div className="p-4">
                ...
                <span
                  className={`inline-block rounded-full px-3 py-1 text-xs font-medium ${CATEGORY_COLORS[item.category]}`}
                >
                  {item.category}
                </span>
              </div>
        ...
        {filtered.length === 0 && (
          <div className="py-16 text-center text-gray-500">
            <p className="text-lg">В этой категории пока нет работ.</p>
          </div>
        )}
        ...
        <div className="mt-16 text-center">
          <div className="card mx-auto max-w-xl p-8 md:p-10">
            <h2 className="heading-3 mb-3">Хотите такой же результат?</h2>
            <p className="mb-6 text-gray-600">
              Отправьте нам фото вашего автомобиля и получите бесплатную
              консультацию.
            </p>
            <a
              href={WHATSAPP_LINK}
